...
public class UsageService {
    ...
    private static void processMessage(Connection db, Message msg) throws SQLException {
        ...
        // Runde Zeit auf volle Stunde
        LocalDateTime timestamp = LocalDateTime.parse(msg.datetime);
        LocalDateTime hour = timestamp.truncatedTo(ChronoUnit.HOURS);

        // Schritt 1: Hole aktuelle Summen für diese Stunde
        String selectSql = "SELECT community_produced, community_used FROM usage_hourly WHERE hour = ?";
        ...
        double newUsed = totalUsed + used;


        double gridUsed = Math.max(0.0, newUsed - newProduced);
        ...
        try (PreparedStatement stmt = db.prepareStatement(upsert)) {
            ...
            stmt.setDouble(4, gridUsed);
            stmt.executeUpdate();

            System.out.printf(" [✓] Updated %s: produced=%.3f, used=%.3f, grid=%.3f%n",
                    hour, newProduced, newUsed, gridUsed);
        }
    }
}
